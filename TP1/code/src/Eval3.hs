...
-- ~ Actualmente W(Const) = 1, W(BTrue) = 1, W(BFalse) = 1
-- Evalua una expresion
evalExp :: Exp a -> State -> Either Error (Pair a State)
evalExp (Const int) state = Right (int :!: addWork 1 state)
...
evalExp BTrue state = Right (True :!: addWork 1 state)
-- ~ evalExp BTrue state = Right (True :!: state)
evalExp BFalse state = Right (False :!: addWork 1 state)
